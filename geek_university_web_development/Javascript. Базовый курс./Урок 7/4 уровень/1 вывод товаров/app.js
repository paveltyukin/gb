...
['phones', 'tablets', 'tv'].forEach((product) => {
    products[product].forEach((productName) => {
        productsElement.insertAdjacentHTML(
          'beforeEnd',
          `
        <div class="product">
          <div>${productName.name}</div>
          <img src="${productName.imageUrl}" alt="">
          <div>${productName.price}</div>
          <a href="https://example.com/producs/${productName.id}">Подробнее</a>
      </div>`
        )
    });
});
